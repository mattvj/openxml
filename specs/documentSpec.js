import openXml from '../src/openxml';
import Ltxml from '../src/ltxml';
...
describe('Document', () => {
  ...
  test('Should create the default document', async done => {
    const document = new openXml.OpenXmlPackage();

    const openDocument = await document.openFromBase64(blankDocument);

    const body = new XElement(W.body);
    const mainDocument = new XElement(W.document,
      new XAttribute(XNamespace.xmlns + 'w', wNs.namespaceName),
      body);

    const xd = openDocument.mainDocumentPart().getXDocument();
    xd.root.replaceWith(mainDocument);

    const base64 = await openDocument.saveToBase64();
  ...
  test('Should create the default document with a run', async done => {
    const document = new openXml.OpenXmlPackage();
  ...
  test('Should create the default document with a paragraph', async done => {
    const document = new openXml.OpenXmlPackage();
  ...
  test('Should create the default document with a paragraph and text', async done => {
    const document = new openXml.OpenXmlPackage();
    ...
    const run = new XElement(W.r);
    const text = new XElement(W.t);
